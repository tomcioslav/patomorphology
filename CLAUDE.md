...
- **Dataset boundary:** `pato.dataset.DatasetViewer` is the only public dataset class. `viewer[i]` returns a `PatoImage`. It's used for two things: **inspection** (notebooks like `explore_images.ipynb`) and as the **input feed** for pipeline-specific torch Datasets (the `sam` pipeline's `SAMFeatureDataset` in `pipelines/sam/data.py`; UNet just feeds a `DatasetViewer` straight into a `DataLoader`). Each pipeline defines whatever torch Datasets it needs in its own `data.py` — they don't go under `pato.dataset` because what counts as "one training sample" differs per pipeline (tile vs SAM-feature vs whatever comes next). For `DataLoader` batching of `PatoImage`-emitting Datasets, pass `collate_fn=PatoImage.collate`.
